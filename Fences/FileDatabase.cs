...
namespace Fences
{
    public class FileDatabase
    {
        ...
        public void GetTotalNumbersFirst(List<Polyline> list)
        {
            Document document = Application.DocumentManager.MdiActiveDocument;
            Database database = document.Database;
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                using (Transaction transaction = database.TransactionManager.StartTransaction())
                {
                    ...
                    if (dbExt.Contains("CustomProp"))
                    {
                        ...
                        numfloor = int.Parse(readBack.Data.AsArray()[0].Value.ToString());
                        numbars = int.Parse(readBack.Data.AsArray()[1].Value.ToString());
                        Properties.Settings.Default.CounterLengthFirst += list[i].Length * numfloor;
                        Properties.Settings.Default.CounterPilsFirst += numfloor * numbars;
                    }
                    transaction.Commit();
                }
